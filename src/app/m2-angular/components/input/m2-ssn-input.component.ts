 ...
 */
@Component({
    selector: 'm2-ssn-input',
    templateUrl: './m2-ssn-input.component.html',
    styleUrls: ['m2-ssn-input.component.scss'],
    providers: [
        {
            provide: MatFormFieldControl,
            useExisting: M2SsnInputComponent
        }]
})
export class M2SsnInputComponent implements ControlValueAccessor, MatFormFieldControl<string>, OnDestroy {
    ...
    @HostBinding()
    id = `m2-ssn-input-${M2SsnInputComponent.nextId++}`;
    ...
    get value(): string | null {
        ...
        if (formGroupValue.part1.length === 3 && formGroupValue.part2.length === 2 && formGroupValue.part3.length === 4) {
            this._errorState = false;
            return formGroupValue.part1 + formGroupValue.part2 + formGroupValue.part3;
        }
        return null;
    }
     ...
     */
    set value(ssn: string | null) {
        if (M2Util.isNullOrEmpty(ssn)) {
            this.formGroup.setValue({part1: '', part2: '', part3: ''});
            return;
        }
        this.formGroup.setValue({part1: ssn.substr(0, 3), part2: ssn.substr(3, 2), part3: ssn.substr(5, 4)});
        this.stateChanges.next();
    }
